...
def index():
    ...
    for symbol_data in symbols:
        ...
        share = db.execute("SELECT Shares FROM {} WHERE Symbol = ?".format(name[0]['username']),sym)
        price = lookup(sym)
        if price == None:
            return render_template("index.html")
        new_price = usd(price["price"])
        new_total = usd(price["price"] * float(share[0]["Shares"]))
        db.execute("UPDATE {} SET Price = ? WHERE Symbol = ?".format(name[0]['username']),new_price,sym)
        db.execute("UPDATE {} SET Total = ? WHERE Symbol = ?".format(name[0]['username']),new_total,sym)
...
def buy():
    ...
    if request.method == 'POST':
        ...
        if symbol in STOCK:
            share_own = db.execute("SELECT Shares FROM {} WHERE Symbol = ?".format(name[0]['username']),symbol)

            share_new = share + int(share_own[0]["Shares"])
            total_new = float(price) * float(share_new)
            db.execute("UPDATE {} SET Shares = ? WHERE Symbol = ?".format(name[0]['username']),share_new,symbol)
            db.execute("UPDATE {} SET Total = ? WHERE Symbol = ?".format(name[0]['username']),total_new,symbol)
        elif symbol not in STOCK:
            db.execute("INSERT INTO {} (Symbol,Name,Price,Shares,Total) VALUES (?,?,?,?,?)".format(name[0]['username']),symbol,symbol,price,share,total)
        db.execute("INSERT INTO {} (State,Symbol,Name,Shares,Price,DAY,MONTH,YEAR,HOUR,MINUTE) VALUES (?,?,?,?,?,?,?,?,?,?)".format(name[0]['username']+"history"),"BUY",symbol,symbol,share,price,day,month,year,hour,minute)
    ...
    if request.method == 'POST':
        ...
        if new_share == 0:
            ...
            if symbol in STOCK:
                ...
                db.execute("UPDATE {} SET Shares = ? WHERE Symbol = ?".format(name[0]['username']),new_share,symbol)
                db.execute("UPDATE {} SET Total = ? WHERE Symbol = ?".format(name[0]['username']),total_new,symbol)
        ...
        db.execute("INSERT INTO {} (State,Symbol,Name,Shares,Price,DAY,MONTH,YEAR,HOUR,MINUTE) VALUES (?,?,?,?,?,?,?,?,?,?)".format(name[0]['username']+"history"),"SELL",symbol,symbol,share,price,day,month,year,hour,minute)
